...
os_name="${ID^} ${VERSION_CODENAME^}"
os_kernel=$({ uname; uname -r; } | tr '\n' ' ')

# Network Information
net_current_user=$(whoami)
net_hostname=$(hostname)
interfaces=$(ip a | awk '/^[a-zA-Z0-9]/ {print $2}')
net_dhcp_ip=$(ip n | awk '{print $1}')
...
ip_address=$(ip addr show $interface | grep -oP '(?<=inet\s)\d+(\.\d+){3}' | head -n 1)
...
printf "├────────────┼───────────────────────────────┤\n"
printf "│ %-10s │ %-29s │\n" "PROCESSOR"  "$cpu_model"
printf "│ %-10s │ %-29s │\n" "CORES"      "$cpu_cores_per_socket CPU(s) / $cpu_sockets Socket(s)"
